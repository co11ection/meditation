...
class WalletTokensView(APIView):
    ...
    def calculate_individual_tokens_to_earn(self, request):
        ...
        try:
            meditation_id = int(request.path.rstrip('/').split('/')[-1])
            group_size = int(request.user.group_meditation.filter(id=meditation_id).first().group_size)
        except:
            group_size = 0
        ...
        individual_tokens = Decimal(str(round(earn_finish, 2)))
